...
安装完成后，终端会显示 `LNMPP安装成功。`、phpMyAdmin 用户名 `root` 和随机生成的 10 位密码（含大小写字母、数字和特殊字符）。登录凭据另保存在仅 root 可读的 `/etc/lnmpp/phpmyadmin-credentials`。phpMyAdmin 位于 `http://服务器IP/phpmyadmin/`，使用 MariaDB 的 `root` 账号登录，具有数据库管理权限。默认禁止非本机 IP 访问 Web 登录；需要时可用下述命令启用远程访问。MariaDB 默认仍只监听本机，脚本没有开放数据库 TCP 远程连接。
...
sudo bash install.sh view phpmyadmin
sudo bash install.sh reset phpmyadmin
```

`stop` 会让外部 IP 访问 LNMPP 管理的默认站点及网站的 `/phpmyadmin` 路径时收到 403；服务器本机的 `127.0.0.1` 和 `::1` 仍可访问，网站其他路径不受影响。`start` 恢复外部访问。命令不会删除账号或数据库，重复执行也有效。`view` 会读取并显示已保存的 phpMyAdmin 密码，不受远程访问开关影响，也不会生成或重置密码。`reset` 会生成新的 10 位随机密码（含大小写字母、数字和特殊字符），同步修改 MariaDB `root` 密码及凭据文件，并显示用户名 `root` 和新密码；它不会改变远程访问开关。已有安装可先在仓库运行 `git pull`，再执行上述命令；切换时会重新生成脚本管理的 Nginx 站点配置，不会修改其他站点配置。
